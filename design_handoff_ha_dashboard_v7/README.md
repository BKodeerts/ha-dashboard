# Handoff: Home Assistant dashboard — adaptive layout + view pass (v7)
...
### Tab bar
- **Three tabs**: Home · Energie · Netwerk. **Auto is removed** (§6).
...
## 4. Instellingen (`7c`, behind the cog)

- `w ≥ 760`: two columns `1fr 1fr`, gap 24. **Left**: Wie ben jij · Wie volg je bovenaan · Overig · reset. **Right**: Kamers sorteren.
- `< 760`: one column in that order.
- Section labels: mono 11, uppercase, 0.1em, `#8a877f`. Notes: mono 10, `#a8a49c`.
- **"Tints per kamer" is removed as a separate section.** The tint swatch now sits in each sort row: an 18 px circle in a 40×44 hit area, and tapping it cycles through `TINT_CYCLE`. This writes `areaTint` like before.
...
- "Overig" rows: Weer · Thema · Kleuren · Opslag in one `#fbfaf7` card (radius 18), 54 px rows, 1 px `#ece9e3` separators, chevron-right `#b4afa5`. Panels expand as in v5.
...
- `screenshots/`: every view at desktop (1020 px) and phone (390 px): `7a-energie-*`, `7b-netwerk-*`, `7c-instellingen-*`, `7d-weer-desktop-modal` (captured at 75%) / `7d-weer-mobiel`, and `6a-home-1020 / -780 / -390` for the room-grid breakpoints.
...
`src/app/App.tsx` (width measurement, breakpoints) · `src/components/RoomGrid.tsx`, `RoomTile.tsx` · `src/components/TabBar.tsx` · `src/components/Sheet.tsx` (modal mode) · `src/components/views/EnergyView.tsx` · `src/ha/energyChart.ts` (drop `selfConsumptionRatio`) · new `src/ha/energyStats.ts` (statistics fetch) · `src/components/views/NetworkView.tsx` · `src/components/views/SettingsView.tsx` · `src/components/sheets/WeatherSheet.tsx` · remove `src/components/views/CarView.tsx` · `src/ui/styles.css`
