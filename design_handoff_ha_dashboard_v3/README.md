# Handoff: HA Dashboard — v3 home screen

## Overview
v3 of the Home Assistant phone dashboard (`BKodeerts/ha-dashboard`). v1 shipped and was rejected on
UX grounds; v2 corrected the structure; v3 compresses the top of the home screen and replaces the
climate panel with a single drag row plus a floating mode picker. This package is the current
specification for the home screen, room card, tab structure, network view and settings panel, and
replaces the v1 and v2 handoffs for everything it covers. `HA_INTEGRATION.md` (the
WebSocket/area-registry/service-call layer) is unchanged and still authoritative.

Three things changed since v2, all of them because the old versions wasted vertical space or sent
unwanted commands:
1. The weather block is a single left-aligned line instead of a centred stack.
2. Presence left the pill row and became a fixed icon+label chip at the top right, so the exception
   pills fit on one line and no longer push the room grid below the fold.
3. The climate panel (power button + mode dropdown + −/+ buttons, ~100px) became one 54px drag row.
   Mode is chosen from a floating popover — **never a cycle**: every mode tap on this system is a
   cloud command to the AC, so a picker that sends exactly one command is a hard requirement.
...
| Centred weather stack + 3 wrapping pills (~210px before the rooms) | One weather line + person chip + one pill row (~110px) |
...
1. **Weather line + person chip** — one row, `padding: 44px 18px 0 20px`, `align-items:center`,
   gap 12px.
   - Left (`flex:1`, taps to the weather sheet): condition icon 24px `#8a877f`, gap 9px,
     temperature `18,0°` at 26px/400 `letter-spacing:-0.03em` `line-height:1`, then hi/lo in 10px
     IBM Plex Mono uppercase `#6e6b64` (`letter-spacing:0.05em`, ellipsis, `white-space:nowrap`):
     `23° / 17°`. The condition word moved into the weather sheet — the icon carries it here.
   - Right, fixed: **person chip**, height 36px, radius 18px, `padding: 0 12px 0 10px`, gap 7px,
     person icon 16px + name 13px/500 + a 6px state dot. Home: bg `oklch(0.68 0.13 250 / 0.16)`,
     fg `oklch(0.42 0.12 250)`, dot `oklch(0.68 0.13 250)`. Away: bg `#e6e3dc`, fg `#8a877f`,
     dot `#b8b4ac`. Shows the person the user did *not* pick as themselves; taps to that person's
     entity detail.
2. **Exception pills** — `padding: 14px 20px 0`, flex, gap 8px, **no wrap, one line only**. Two
   pills: alarm and openings. Height 40px, radius 20px, `padding: 0 14px`, icon 17px (`flex:0 0 auto`)
   + label 14px/500 with `overflow:hidden; text-overflow:ellipsis; white-space:nowrap`. The openings
   label is count-based so it can never wrap: `1 raam open` / `3 ramen open` / `Alles dicht` —
   ...
   - Labels: `Alarm uit` / `Alarm thuis` / `Alarm afwezig`.
...
sheet closes), `tab` (`home` | `energie` | `netwerk` | `auto` | `meer`), per-room light
...
- Radii: 28 sheet, 24 popover, 20 tile, 20 pill, 19 popover option, 18 person chip, 16 row, 14 button,
  12 square control, 10 chip, 8 state chip.
...
- Motion: `sheetUp 0.28s cubic-bezier(0.2,0.9,0.2,1)`, `fadeIn 0.18s ease`, mode picker
  `fadeIn 0.14s ease`, brightness and setpoint fill `width 0.09s linear`.
...
- `Home Dashboard.dc.html` — the prototype. The **top section only** (badge `3b`, the light screen)
  is the v3 spec; the sections below it are earlier explorations and should be ignored.
