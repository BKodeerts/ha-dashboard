...
1. **Top row** — `padding:52px 24px 0`, flex, `align-items:center`, `gap:8px`.
   - Left: date, IBM Plex Mono 11px, `letter-spacing:0.12em`, uppercase, `#a8a49c`. Copy: "Woensdag 18 augustus".
   - Right: two 36×36 circular buttons, `background:#e6e3dc`, icon 17px `#57544e`, `gap:8px`: the alarm control (mdi shield-home) and settings (mdi cog-outline). The alarm button keeps its v4 behaviour — Weg/Nacht/Uit picker, pulsing amber dot during exit delay. Settings opens the same screen the v4 gear opened from the Kamers row; it moved up here because a lone gear on the section row read as clutter.
2. **Weather + person row** — `padding:20px 24px 0`, flex, `align-items:center`, `gap:9px`. The weather deliberately lost its hero treatment: it is reference information, not the reason the screen exists.
   - Left: weather glyph **20×20** fill `#b4afa5`; temperature 22px / 400 / `-0.03em` ("18,0°"); range in IBM Plex Mono 11px `0.08em` uppercase `#a8a49c` ("23 / 17"). No condition text.
   - Right (pushed with a `flex:1` spacer): **person chip**, height 34px, `padding:0 12px 0 3px`, `border-radius:17px`, `background:#e6e3dc`, text `#3b3934`. Inside: a 28px circle `background:#f0eeea` with a 15px account glyph `#57544e`, then the zone in IBM Plex Mono 11px `0.08em` uppercase ("WERK 14 KM"). Exactly one person is tracked — this is never a list. The zone plus distance is the point: it answers "still at work, or nearly home". At home the copy is just "THUIS" with no distance. Tap opens the person's detail.
3. **Section row** — `padding:34px 24px 0`, flex, `align-items:center`, `min-height:44px`.
...
4. **Room grid** — `flex:1`, `padding:18px 24px 110px`, two equal columns, `gap:16px`.
   ...
   - "4 meer" tile: dashed `1px #d8d4cc`, radius 24px, IBM Plex Mono 10px `0.08em` uppercase `#a8a49c`, chevron-down 14px. **Its span depends on parity**: with an odd number of visible room cards it takes the leftover half-slot; with an even number (e.g. six favourites) it spans both columns at `min-height:56px` so it can never be pushed out of view. Same rule when expanded.
...
The top row and the weather + person row persist (they are treated as screen-level, not home-only). Section label becomes "Nu"; the windows chip is hidden here.
...
### Header variants (turn 5)
The top section of the options file holds three smaller person/zone treatments explored before landing on the current header: 5a (chip on the weather line), 5b (plain text line under the date), 5c (person as a button in the circle row). The built header is **5a's layout with 5c's filled-chip styling** — that combination is the one described above. The crops are reference only.
...
- **Person chip**: tap opens the tracked person's detail. Copy is zone + distance away from home, zone only when home.
...
- Text primary `#111318`; secondary `#4a4842` / `#3b3934`; muted `#8a877f`; faint `#a8a49c`; inactive / weather glyph `#b4afa5`
...
Spacing — screen inset 24px (v4 used 20px); grid gap 16px; top row top 52px; weather + person row top 20px; section row top 34px; grid top 18px; bottom padding 110px to clear the tab bar.

Typography — Space Grotesk 400/500/700 for UI, IBM Plex Mono 400/500 for labels, units and metadata. Scale: 36 / 28 / 22 / 17 / 16 / 15 / 14 (Grotesk) and 14 / 11 / 10 (Mono). Negative tracking on large numbers (`-0.04em` at 36px, `-0.035em` at 28px, `-0.03em` at 22px); mono labels are uppercase with `0.08em`–`0.12em`.

Radii — 44 phone, 24 card, 22 chip / tab bar, 19 small chip, 17 person chip, 16 tab item, 14 avatar, 3 progress bar. Tint bar 4px wide, full card height.
...
## Files
- `Home Ruimte Opties.dc.html` — turn 5 header crops on top, the three layout options below; **4a is the design to build**
